...
if __name__ == "__main__":
    ...
    parser.add_argument("-S", "--taille_min", required=False, type=int, default=2,
    ...
    parser.add_argument("-l", "--location", required=False, type=str,
                        help="Represente le nom dossier dans lequel les exemples vont être créés")
    ...
    path = args.location if args.location else f'ex/s{taille_min}-t{nb_tailles}-n{nb_exemplaires}-r{args.random_seed}'
    Path(path).mkdir(parents=True, exist_ok=True)

    # Generate
    for i in range(nb_exemplaires):
        for j in range(taille_min, taille_min+nb_tailles):
            with open(f'{path}/ex{j}_{i}', 'w') as inst:
                inst.write(f"{j}\n")
                for k in range(2**j):
